...
// 旧的 didk33e0/popup 路径已被上游弃用，会返回 verify_code F019。
const SCENE = process.argv[2] || '36qgs6xb';
const REGION = process.argv[3] || 'sgp';
const PREFIX = process.argv[4] || 'no8xfe';
const MODE = process.argv[5] || 'embed';
...
(async () => {
  await waitFor(() => typeof window.initAliyunCaptcha === 'function', 30000);
