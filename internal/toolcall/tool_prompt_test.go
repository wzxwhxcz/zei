...
func TestBuildToolCallInstructions_ExecCommandUsesCmdExample(t *testing.T) {
	...
	if !strings.Contains(out, `<|GLMML|invoke name="exec_command">`) {
		t.Fatalf("expected exec_command in examples, got: %s", out)
	}
	if !strings.Contains(out, `<|GLMML|parameter name="cmd"><![CDATA[pwd]]></|GLMML|parameter>`) {
...
func TestBuildToolCallInstructions_ExecuteCommandUsesCommandExample(t *testing.T) {
	...
	if !strings.Contains(out, `<|GLMML|invoke name="execute_command">`) {
		t.Fatalf("expected execute_command in examples, got: %s", out)
	}
	if !strings.Contains(out, `<|GLMML|parameter name="command"><![CDATA[pwd]]></|GLMML|parameter>`) {
...
func TestBuildToolCallInstructions_BashUsesCommandAndDescriptionExamples(t *testing.T) {
	...
	for _, block := range blocks {
		if !strings.Contains(block, `<|GLMML|parameter name="command">`) {
			t.Fatalf("expected every Bash example to use command parameter, got: %s", block)
		}
		if strings.Contains(block, `<|GLMML|parameter name="path">`) || strings.Contains(block, `<|GLMML|parameter name="content">`) {
			t.Fatalf("expected Bash examples not to use file write parameters, got: %s", block)
		}
		if strings.Contains(block, `<|GLMML|parameter name="description">`) {
			sawDescription = true
		}
	}
	if !sawDescription {
	...
	if strings.Contains(out, `<|GLMML|invoke name="Read">`) {
...
func TestBuildToolCallInstructions_ExecCommandLongScriptUsesCmd(t *testing.T) {
	...
	for _, block := range blocks {
		if !strings.Contains(block, `<|GLMML|parameter name="cmd">`) {
			t.Fatalf("expected exec_command examples to use cmd parameter, got: %s", block)
		}
		if strings.Contains(block, `<|GLMML|parameter name="command">`) || strings.Contains(block, `<|GLMML|parameter name="path">`) || strings.Contains(block, `<|GLMML|parameter name="content">`) {
...
func TestBuildToolCallInstructions_WriteUsesFilePathAndContent(t *testing.T) {
	...
	for _, block := range blocks {
		if !strings.Contains(block, `<|GLMML|parameter name="file_path">`) || !strings.Contains(block, `<|GLMML|parameter name="content">`) {
			t.Fatalf("expected Write examples to use file_path and content, got: %s", block)
		}
		if strings.Contains(block, `<|GLMML|parameter name="path">`) {
...
func TestBuildToolCallInstructions_AnchorsMissingOpeningWrapperFailureMode(t *testing.T) {
	...
	if !strings.Contains(out, "Never omit the opening <|GLMML|tool_calls> tag") {
...
func findInvokeBlocks(text, name string) []string {
	...
	for {
		...
		end := strings.Index(remaining, `</|GLMML|invoke>`)
		if end < 0 {
			return blocks
		}
		end += len(`</|GLMML|invoke>`)
