...
func TestParseToolCallsSupportsGLMMLShell(t *testing.T) {
	text := `<|GLMML|tool_calls><|GLMML|invoke name="Bash"><|GLMML|parameter name="command"><![CDATA[pwd]]></|GLMML|parameter></|GLMML|invoke></|GLMML|tool_calls>`
	calls := ParseToolCalls(text, []string{"Bash"})
	if len(calls) != 1 {
		t.Fatalf("expected 1 GLMML call, got %#v", calls)
	}
	if calls[0].Name != "Bash" || calls[0].Input["command"] != "pwd" {
		t.Fatalf("unexpected GLMML parse result: %#v", calls[0])
	}
}

func TestParseToolCallsSupportsHyphenatedGLMMLShellWithHereDocCDATA(t *testing.T) {
	text := `<glmml-tool-calls>
<glmml-invoke name="Bash">
<glmml-parameter name="command"><![CDATA[git commit -m "$(cat <<'EOF'
...
<glmml-parameter name="description"><![CDATA[Create commit with architecture doc updates]]></glmml-parameter>
</glmml-invoke>
</glmml-tool-calls>`
	calls := ParseToolCalls(text, []string{"Bash"})
	if len(calls) != 1 {
		t.Fatalf("expected 1 hyphenated GLMML call, got %#v", calls)
	}
	if calls[0].Name != "Bash" {
...
func TestParseToolCallsSupportsUnderscoredGLMMLShell(t *testing.T) {
	text := `<glmml_tool_calls>
<glmml_invoke name="search_web">
<glmml_parameter name="query"><![CDATA[2026年5月 热点事件]]></glmml_parameter>
<glmml_parameter name="topic"><![CDATA[news]]></glmml_parameter>
</glmml_invoke>
<glmml_invoke name="eval_javascript">
<glmml_parameter name="code"><![CDATA[1 + 1]]></glmml_parameter>
</glmml_invoke>
</glmml_tool_calls>`
	calls := ParseToolCalls(text, []string{"search_web", "eval_javascript"})
	if len(calls) != 2 {
		t.Fatalf("expected two underscored GLMML calls, got %#v", calls)
	}
	if calls[0].Name != "search_web" || calls[0].Input["query"] != "2026年5月 热点事件" || calls[0].Input["topic"] != "news" {
		t.Fatalf("unexpected first underscored GLMML call: %#v", calls[0])
	}
	if calls[1].Name != "eval_javascript" || calls[1].Input["code"] != "1 + 1" {
		t.Fatalf("unexpected second underscored GLMML call: %#v", calls[1])
	}
}
...
func TestParseToolCallsSupportsFullwidthGLMMLShell(t *testing.T) {
...
func TestParseToolCallsSupportsFullwidthBangGLMMLDrift(t *testing.T) {
	text := `<！GLMML！tool_calls>
  <！GLMML！invoke name=“Bash”>
  <！GLMML！parameter name=“command”><！[CDATA[lsof -i :4321 -t]]><！/GLMML！parameter>
  <！GLMML！parameter name=“description”><！[CDATA[Verify port 4321 is free]]><！/GLMML！parameter>
  <！/GLMML！invoke>
  <！/GLMML！tool_calls>`

	calls := ParseToolCalls(text, []string{"Bash"})
	if len(calls) != 1 {
		t.Fatalf("expected one fullwidth-bang GLMML drift call, got %#v", calls)
	}
	if calls[0].Name != "Bash" || calls[0].Input["command"] != "lsof -i :4321 -t" || calls[0].Input["description"] != "Verify port 4321 is free" {
		t.Fatalf("unexpected fullwidth-bang GLMML drift call: %#v", calls[0])
	}
}

func TestParseToolCallsSupportsIdeographicCommaGLMMLDrift(t *testing.T) {
	text := `<、GLMML、tool_calls>
  <、GLMML、invoke name="Bash">
    <、GLMML、parameter name="command"><、[CDATA[git commit -m "$(cat <<'EOF'
feat: expand fullwidth bang separator and curly quote tolerance in GLMML tool parsing

Co-Authored-By: Claude Opus 4.6 noreply@anthropic.com
EOF
)"]]><、/GLMML、parameter>
    <、GLMML、parameter name="description"><、[CDATA[Create commit with staged changes]]><、/GLMML、parameter>
  <、/GLMML、invoke>
<、/GLMML、tool_calls>`

	calls := ParseToolCalls(text, []string{"Bash"})
	if len(calls) != 1 {
		t.Fatalf("expected one ideographic-comma GLMML drift call, got %#v", calls)
	}
	command, _ := calls[0].Input["command"].(string)
	if calls[0].Name != "Bash" || !strings.Contains(command, `git commit -m "$(cat <<'EOF'`) || !strings.Contains(command, "Co-Authored-By: Claude Opus 4.6 noreply@anthropic.com") {
		t.Fatalf("unexpected ideographic-comma GLMML drift call: %#v", calls[0])
	}
...
func TestParseToolCallsToleratesGLMMLTrailingPipeTagTerminator(t *testing.T) {
	text := strings.Join([]string{
		`<|GLMML|tool_calls| `,
		`  <|GLMML|invoke name="terminal">`,
		`    <|GLMML|parameter name="command"><![CDATA[find "/home" -type d]]></|GLMML|parameter>`,
		`    <|GLMML|parameter name="timeout"><![CDATA[10]]></|GLMML|parameter>`,
		`  </|GLMML|invoke>`,
		`</|GLMML|tool_calls>`,
	}, "\n")
	calls := ParseToolCalls(text, []string{"terminal"})
	if len(calls) != 1 {
		t.Fatalf("expected one trailing-pipe GLMML call, got %#v", calls)
	}
...
func TestParseToolCallsToleratesGLMMLTrailingNovelSeparatorTagTerminator(t *testing.T) {
	text := strings.Join([]string{
		`<GLMMLtool_calls※>`,
		`  <GLMMLinvoke name="Bash"※>`,
		`    <GLMMLparameter name="command"※><![CDATA[pwd]]></GLMMLparameter※>`,
		`  </GLMMLinvoke※>`,
		`</GLMMLtool_calls※>`,
	}, "\n")
	calls := ParseToolCalls(text, []string{"Bash"})
	if len(calls) != 1 {
		t.Fatalf("expected one trailing-separator GLMML call, got %#v", calls)
	}
	if calls[0].Name != "Bash" || calls[0].Input["command"] != "pwd" {
		t.Fatalf("unexpected trailing-separator GLMML parse result: %#v", calls[0])
	}
}

func TestParseToolCallsToleratesExtraLeadingLessThanBeforeGLMML(t *testing.T) {
	text := `<<|GLMML|tool_calls><<|GLMML|invoke name="Bash"><<|GLMML|parameter name="command"><![CDATA[pwd]]></|GLMML|parameter></|GLMML|invoke></|GLMML|tool_calls>`
	calls := ParseToolCalls(text, []string{"Bash"})
	if len(calls) != 1 {
		t.Fatalf("expected one extra-leading-less-than GLMML call, got %#v", calls)
	}
	if calls[0].Name != "Bash" || calls[0].Input["command"] != "pwd" {
		t.Fatalf("unexpected extra-leading-less-than GLMML parse result: %#v", calls[0])
	}
}

func TestParseToolCallsToleratesRepeatedGLMMLPrefixNoise(t *testing.T) {
	text := `<<GLMML|GLMML|tool_calls><<GLMML|GLMML|invoke name="Bash"><<GLMML|GLMML|parameter name="command"><![CDATA[git status]]></GLMML|GLMML|parameter></GLMML|GLMML|invoke></GLMML|GLMML|tool_calls>`
	calls := ParseToolCalls(text, []string{"Bash"})
	if len(calls) != 1 {
		t.Fatalf("expected one repeated-prefix GLMML call, got %#v", calls)
	}
	if calls[0].Name != "Bash" || calls[0].Input["command"] != "git status" {
		t.Fatalf("unexpected repeated-prefix GLMML parse result: %#v", calls[0])
	}
}

func TestParseToolCallsSupportsGLMMLShellWithCanonicalExampleInCDATA(t *testing.T) {
	content := `<tool_calls><invoke name="demo"><parameter name="value">x</parameter></invoke></tool_calls>`
	text := `<|GLMML|tool_calls><|GLMML|invoke name="Write"><|GLMML|parameter name="file_path">notes.md</|GLMML|parameter><|GLMML|parameter name="content"><![CDATA[` + content + `]]></|GLMML|parameter></|GLMML|invoke></|GLMML|tool_calls>`
	calls := ParseToolCalls(text, []string{"Write"})
	if len(calls) != 1 {
		t.Fatalf("expected 1 GLMML call with XML-looking CDATA, got %#v", calls)
	}
	if calls[0].Name != "Write" || calls[0].Input["content"] != content {
		t.Fatalf("unexpected GLMML CDATA parse result: %#v", calls[0])
	}
}

func TestParseToolCallsKeepsHereDocCDATAWithFencedGLMMLAndLiteralCDATAEnd(t *testing.T) {
	command := strings.Join([]string{
		...
		`<|GLMML|tool_calls>`,
		`  <|GLMML|invoke name="Bash">`,
		`    <|GLMML|parameter name="command"><![CDATA[grep -E "error|fail" < input.log 2>&1]]></|GLMML|parameter>`,
		`  </|GLMML|invoke>`,
		`</|GLMML|tool_calls>`,
		"```",
		"",
	...
	text := `<|GLMML|tool_calls><|GLMML|invoke name="Bash"><|GLMML|parameter name="command"><![CDATA[` + command + `]]></|GLMML|parameter><|GLMML|parameter name="description"><![CDATA[Write project value doc]]></|GLMML|parameter></|GLMML|invoke></|GLMML|tool_calls>`

	calls := ParseToolCalls(text, []string{"Bash"})
	if len(calls) != 1 {
		t.Fatalf("expected one GLMML call with extreme heredoc CDATA, got %#v", calls)
	}
...
func TestParseToolCallsKeepsCompactCDATAWithImmediateFencedGLMML(t *testing.T) {
	content := strings.Join([]string{
		"```xml",
		`<|GLMML|tool_calls>`,
		`  <|GLMML|invoke name="Bash">`,
		`    <|GLMML|parameter name="command"><![CDATA[echo compact]]></|GLMML|parameter>`,
		`  </|GLMML|invoke>`,
		`</|GLMML|tool_calls>`,
		"```",
		"tail",
...
func TestParseToolCallsNormalizesMixedGLMMLAndCanonicalToolTags(t *testing.T) {
	// Models commonly mix GLMML wrapper tags with canonical inner tags.
	// These should be normalized and parsed, not rejected.
	text := `<|GLMML|tool_calls><invoke name="Bash"><|GLMML|parameter name="command">pwd</|GLMML|parameter></invoke></|GLMML|tool_calls>`
	calls := ParseToolCalls(text, []string{"Bash"})
	if len(calls) != 1 {
		t.Fatalf("expected mixed GLMML/XML tool tags to be normalized and parsed, got %#v", calls)
	}
	if calls[0].Name != "Bash" || calls[0].Input["command"] != "pwd" {
		t.Fatalf("unexpected mixed GLMML parse result: %#v", calls[0])
	}
}
...
func TestParseToolCallsTreatsItemOnlyParameterBodyAsArray(t *testing.T) {
	text := strings.Join([]string{
		`<|GLMML|tool_calls>`,
		`<|GLMML|invoke name="AskUserQuestion">`,
		`<|GLMML|parameter name="questions">`,
		`<item>`,
...
func TestParseToolCallsTreatsCDATAItemOnlyBodyAsArray(t *testing.T) {
	...
	text := `<|GLMML|tool_calls><|GLMML|invoke name="TodoWrite"><|GLMML|parameter name="todos"><![CDATA[` + todos + `]]></|GLMML|parameter></|GLMML|invoke></|GLMML|tool_calls>`
...
func TestParseToolCallsToleratesGLMMLSpaceSeparatorTypo(t *testing.T) {
	text := strings.Join([]string{
		"<|GLMML tool_calls>",
		"<|GLMML invoke name=\"Read\">",
		"<|GLMML parameter name=\"file_path\"><![CDATA[/tmp/input.txt]]></|GLMML parameter>",
		"</|GLMML invoke>",
		"</|GLMML tool_calls>",
	}, "\n")
	calls := ParseToolCalls(text, []string{"Read"})
	if len(calls) != 1 {
		t.Fatalf("expected one call from GLMML space-separator typo, got %#v", calls)
	}
...
func TestParseToolCallsDoesNotAcceptGLMMLSpaceLookalikeTagName(t *testing.T) {
	text := strings.Join([]string{
		"<|GLMML tool_calls_extra>",
		"<|GLMML invoke name=\"Read\">",
		"<|GLMML parameter name=\"file_path\">/tmp/input.txt</|GLMML parameter>",
		"</|GLMML invoke>",
		"</|GLMML tool_calls_extra>",
	}, "\n")
	calls := ParseToolCalls(text, []string{"Read"})
...
func TestParseToolCallsToleratesGLMMLCollapsedTagNames(t *testing.T) {
	todos := `[x] 检查 toolcalls_format.go 格式化逻辑
[x] 检查 toolcalls_parse.go 解析逻辑
[x] 检查 toolcalls_xml.go 和 toolcalls_glmml.go
...
[x] 给出调查结论`
	text := strings.Join([]string{
		"[]",
		"<GLMMLtool_calls>",
		"<GLMMLinvoke name=\"update_todo_list\">",
		"<GLMMLparameter name=\"todos\"><![CDATA[" + todos + "]]></GLMMLparameter>",
		"</GLMMLinvoke>",
		"</GLMMLtool_calls>",
	}, "\n")
	calls := ParseToolCalls(text, []string{"update_todo_list"})
	if len(calls) != 1 {
		t.Fatalf("expected one call from collapsed GLMML tags, got %#v", calls)
	}
...
func TestParseToolCallsDoesNotAcceptGLMMLCollapsedLookalikeTagName(t *testing.T) {
	text := strings.Join([]string{
		"<GLMMLtool_calls_extra>",
		"<GLMMLinvoke name=\"update_todo_list\">",
		"<GLMMLparameter name=\"todos\">x</GLMMLparameter>",
		"</GLMMLinvoke>",
		"</GLMMLtool_calls_extra>",
	}, "\n")
	calls := ParseToolCalls(text, []string{"update_todo_list"})
...
func TestParseToolCallsSkipsProseMentionOfSameWrapperVariant(t *testing.T) {
	text := strings.Join([]string{
		"Summary: support canonical <tool_calls> and GLMML <|GLMML|tool_calls> wrappers.",
		"",
		"<|GLMML|tool_calls>",
		"<|GLMML|invoke name=\"Bash\">",
		"<|GLMML|parameter name=\"command\"><![CDATA[git status]]></|GLMML|parameter>",
		"</|GLMML|invoke>",
		"</|GLMML|tool_calls>",
	}, "\n")
	res := ParseToolCallsDetailed(text, []string{"Bash"})
...
func TestParseToolCallsSupportsGLMMLShellWithFullwidthClosingSlash(t *testing.T) {
	text := `<|GLMML|tool_calls><|GLMML|invoke name="execute_code"><|GLMML|parameter name="code"><![CDATA[print("hi")]]></|GLMML|parameter></|GLMML|invoke><／GLMML|tool_calls>`
	calls := ParseToolCalls(text, []string{"execute_code"})
	if len(calls) != 1 {
		t.Fatalf("expected 1 GLMML call with fullwidth closing slash, got %#v", calls)
	}
	if calls[0].Name != "execute_code" || calls[0].Input["code"] != `print("hi")` {
		t.Fatalf("unexpected fullwidth-closing-slash GLMML parse result: %#v", calls[0])
	}
}

func TestParseToolCallsSupportsGLMMLShellWithSentencePieceSeparatorAndFullwidthGT(t *testing.T) {
	text := `<|GLMML▁tool_calls|><|GLMML▁invoke▁name="execute_code"><|GLMML▁parameter▁name="code"><![CDATA[print("hi")]]></|GLMML▁parameter></|GLMML▁invoke></|GLMML▁tool_calls＞`
	calls := ParseToolCalls(text, []string{"execute_code"})
	if len(calls) != 1 {
		t.Fatalf("expected 1 GLMML call with sentencepiece separator and fullwidth terminator, got %#v", calls)
	}
	if calls[0].Name != "execute_code" || calls[0].Input["code"] != `print("hi")` {
		t.Fatalf("unexpected sentencepiece/fullwidth-terminator GLMML parse result: %#v", calls[0])
	}
}

func TestParseToolCallsSupportsGLMMLShellWithFullwidthLTUnicodeSpaceAndFullwidthAttributes(t *testing.T) {
	text := `＜|GLMML　tool_calls＞＜|GLMML　invoke　name＝“execute_code”＞＜|GLMML　parameter　name＝“code”＞<![CDATA[print("hi")]]>＜／GLMML|parameter＞＜／GLMML|invoke＞＜／GLMML|tool_calls＞`
	calls := ParseToolCalls(text, []string{"execute_code"})
	if len(calls) != 1 {
		t.Fatalf("expected 1 GLMML call with fullwidth opening delimiter and Unicode attribute confusables, got %#v", calls)
	}
	if calls[0].Name != "execute_code" || calls[0].Input["code"] != `print("hi")` {
		t.Fatalf("unexpected fullwidth-opening/Unicode-attr GLMML parse result: %#v", calls[0])
	}
}

func TestParseToolCallsCanonicalizesConfusableCandidateShellOnly(t *testing.T) {
	text := "<|\u200b\uff24\u0405\u039cL|to\u03bfl\uff3fcalls>" +
		"<|\ufeffGLMML|inv\u03bfk\u0435 n\u0430me\uff1d\u201cexecute_code\u201d>" +
		"<|\u200bGLMML|par\u0430meter n\u0430me\uff1d\u201ccode\u201d><![\ufeff\u0421D\u0410T\u0410[print(\"hi\")]]>" +
		"</|\u200bGLMML|par\u0430meter></|\u200bGLMML|inv\u03bfk\u0435></|\u200b\uff24\u0405\u039cL|to\u03bfl\uff3fcalls>"
	calls := ParseToolCalls(text, []string{"execute_code"})
	if len(calls) != 1 {
