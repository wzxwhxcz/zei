...
<|GLMML|tool_calls>
  <|GLMML|invoke name="TOOL_NAME_HERE">
    <|GLMML|parameter name="PARAMETER_NAME"><![CDATA[PARAMETER_VALUE]]></|GLMML|parameter>
  </|GLMML|invoke>
</|GLMML|tool_calls>

RULES:
1) Use the <|GLMML|tool_calls> wrapper format.
2) Put one or more <|GLMML|invoke> entries under a single <|GLMML|tool_calls> root.
3) Put the tool name in the invoke name attribute: <|GLMML|invoke name="TOOL_NAME">.
3a) Tag punctuation alphabet: ASCII < > / = " plus the halfwidth pipe |.
4) All string values must use <![CDATA[...]]>, even short ones. This includes code, scripts, file contents, prompts, paths, names, and queries.
5) Every top-level argument must be a <|GLMML|parameter name="ARG_NAME">...</|GLMML|parameter> node.
...
13) If you call a tool, the first non-whitespace characters of that tool block must be exactly <|GLMML|tool_calls>.
14) Never omit the opening <|GLMML|tool_calls> tag, even if you already plan to close with </|GLMML|tool_calls>.
15) Compatibility note: the runtime also accepts the legacy XML tags <tool_calls> / <invoke> / <parameter>, but prefer the GLMML-prefixed form above.

PARAMETER SHAPES:
- string => <|GLMML|parameter name="x"><![CDATA[value]]></|GLMML|parameter>
- object => <|GLMML|parameter name="x"><field>...</field></|GLMML|parameter>
- array => <|GLMML|parameter name="x"><item>...</item><item>...</item></|GLMML|parameter>
- number/bool/null => <|GLMML|parameter name="x">plain_text</|GLMML|parameter>

【WRONG — Do NOT do these】:

Wrong 1 — mixed text after XML:
  <|GLMML|tool_calls>...</|GLMML|tool_calls> I hope this helps.
Wrong 2 — Markdown code fences:
  ` + "```xml" + `
  <|GLMML|tool_calls>...</|GLMML|tool_calls>
  ` + "```" + `
Wrong 3 — missing opening wrapper:
  <|GLMML|invoke name="TOOL_NAME">...</|GLMML|invoke>
  </|GLMML|tool_calls>
Wrong 4 — empty parameters:
  <|GLMML|tool_calls>
    <|GLMML|invoke name="Bash">
      <|GLMML|parameter name="command"></|GLMML|parameter>
    </|GLMML|invoke>
  </|GLMML|tool_calls>

Remember: The ONLY valid way to use tools is the <|GLMML|tool_calls>...</|GLMML|tool_calls> block at the end of your response.
...
func renderToolExampleBlock(calls []promptToolExample) string {
	...
	b.WriteString("<|GLMML|tool_calls>\n")
	for _, call := range calls {
		b.WriteString(`  <|GLMML|invoke name="`)
...
func indentPromptParameters(body, indent string) string {
	if strings.TrimSpace(body) == "" {
		return indent + `<|GLMML|parameter name="content"></|GLMML|parameter>`
	}
...
func wrapParameter(name, inner string) string {
	return `<|GLMML|parameter name="` + name + `">` + inner + `</|GLMML|parameter>`
}
...
func exampleBasicParams(name string) (string, bool) {
	...
	case "MultiEdit":
		return wrapParameter("file_path", promptCDATA("README.md")) + "\n" + `<|GLMML|parameter name="edits"><item><old_string>` + promptCDATA("foo") + `</old_string><new_string>` + promptCDATA("bar") + `</new_string></item></|GLMML|parameter>`, true
	}
	return "", false
}
...
func exampleNestedParams(name string) (string, bool) {
	...
	case "ask_followup_question":
		return wrapParameter("question", promptCDATA("Which approach do you prefer?")) + "\n" + `<|GLMML|parameter name="follow_up"><item><text>` + promptCDATA("Option A") + `</text></item><item><text>` + promptCDATA("Option B") + `</text></item></|GLMML|parameter>`, true
	}
	return "", false
}
