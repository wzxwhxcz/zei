...
func normalizeGLMMLToolCallMarkup(text string) (string, bool) {
	if text == "" {
		return "", true
	}
	canonicalized := canonicalizeToolCallCandidateSpans(text)
	hasGLMMLLikeMarkup, hasCanonicalMarkup := ContainsToolMarkupSyntaxOutsideIgnored(canonicalized)
	if !hasGLMMLLikeMarkup && !hasCanonicalMarkup {
		return canonicalized, true
	}
	return rewriteGLMMLToolMarkupOutsideIgnored(canonicalized), true
}

func rewriteGLMMLToolMarkupOutsideIgnored(text string) string {
	if text == "" {
		return ""
	}
