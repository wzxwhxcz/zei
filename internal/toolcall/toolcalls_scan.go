...
var toolMarkupNames = []toolMarkupNameAlias{
	...
	{raw: "tool-calls", canonical: "tool_calls", glmmlOnly: true},
	{raw: "toolcalls", canonical: "tool_calls", glmmlOnly: true},
...
func ContainsToolMarkupSyntaxOutsideIgnored(text string) (hasGLMML, hasCanonical bool) {
	for i := 0; i < len(text); {
		next, advanced, blocked := skipXMLIgnoredSection(text, i)
		if blocked {
			return hasGLMML, hasCanonical
		}
		if advanced {
			i = next
			continue
...
func ContainsToolCallWrapperSyntaxOutsideIgnored(text string) (hasGLMML, hasCanonical bool) {
	for i := 0; i < len(text); {
		next, advanced, blocked := skipXMLIgnoredSection(text, i)
		if blocked {
			return hasGLMML, hasCanonical
...
func scanToolMarkupTagAt(text string, start int) (ToolMarkupTag, bool) {
	...
	prefixStart := i
	i, glmmlLike := consumeToolMarkupNamePrefix(text, i)
	name, nameLen := matchToolMarkupName(text, i, glmmlLike)
	if nameLen == 0 {
...
func IsPartialToolMarkupTagPrefix(text string) bool {
	...
	for i <= len(text) {
		...
		if hasASCIIPartialPrefixFoldAt(text, i, "glmml") {
			return true
		}
...
func consumeToolMarkupNamePrefixOnce(text string, idx int) (int, bool) {
	...
	if next, ok := consumeToolKeyword(text, idx, "glmml"); ok {
...
func matchToolMarkupName(text string, start int, glmmlLike bool) (string, int) {
	for _, name := range toolMarkupNames {
		if name.glmmlOnly && !glmmlLike {
			continue
		}
...
func hasPartialToolMarkupNameAfterArbitraryPrefix(text string, start int) bool {
	for idx := start; idx < len(text); {
		...
		if toolMarkupPrefixAllowsLocalNameAt(text, start, idx) && hasGLMMLNamePrefixOrPartial(text, idx) {
			return true
		}
		_, size := utf8.DecodeRuneInString(text[idx:])
		if size <= 0 {
...
func hasGLMMLNamePrefixOrPartial(text string, start int) bool {
	return hasASCIIPrefixFoldAt(text, start, "glmml") || hasASCIIPartialPrefixFoldAt(text, start, "glmml")
}
...
func toolMarkupPrefixAllowsLocalName(prefix string) bool {
	...
	if strings.Contains(normalizedASCIILowerString(prefix), "glmml") {
		return true
	}
	if strings.ContainsAny(prefix, "=\"'") {
		return false
